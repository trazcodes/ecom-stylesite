...
function MainCategoryProduct() {
    ...
    const getProductByMainCat = async () => {
        try {
            const { data } = await axios.get(`http://localhost:8080/api/category-product/all/${params.mainCategory}`);
    ...
    const getAllCategory = async () => {
        try {
            ...
            setCategories(resp.data?.category.filter(c => c.mainCategory === params.mainCategory));
        } catch (error) {
    ...
    const productFilter = async (checkedCategories, priceRange) => {
        try {
            ...
            setProducts(data?.products.filter(p => p.category.mainCategory === params.mainCategory));
        } catch (error) {
            console.log(error);
        }
    }
    ...
    return (
        <div className='container-fluid mt-3 p-3'>
            <div className='row'>
                <h1 className='text-center mt-3 mb-4 text-uppercase'>{params.mainCategory.charAt(0).toUpperCase() + params.mainCategory.slice(1)}'s Collection</h1>
